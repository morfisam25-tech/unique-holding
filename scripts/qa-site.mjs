...
for(const token of [
  ...
  '+90 212 727 22 22','sales@uniqueholding.com.tr',
...
if(sha256(corporateFooter)!=='68debc46cff05e64fbe6379e9e1bad2167cb57c658b1131525a361181d4ecb45')errors.push('corporate.html: protected Phase 03 static footer changed');
...
// Phase 11 — Industrial Sales / RFQ experience guards.
{
  ...
  const mailAddresses=[...sales.matchAll(/mailto:([^?"']+)/gi)].map(m=>m[1].toLowerCase());for(const address of mailAddresses)if(!['sales@uniqueholding.com.tr'].includes(address))errors.push('sales.html: invented email address -> '+address);
...
// Phase 15 — Venture portfolio / YEKI HAST maturity guards.
{
  const ventures=read('ventures.html');const venturesMain=ventures.match(/<main\b[\s\S]*?<\/main>/i)?.[0]||'';const venturesCss=read('assets/ventures.css');
  if(!ventures.includes('assets/ventures.css')||!venturesCss.startsWith('@layer page{'))errors.push('Phase 15: ventures stylesheet architecture missing');
  if((venturesMain.match(/<h1\b/gi)||[]).length!==1)errors.push('ventures.html: Phase 15 requires exactly one H1');
  for(const token of ['01 / Venture Development','STATUS-LED PORTFOLIO','02 / Portfolio Status','03 / Evidence Axis','04 / YEKI HAST','05 / Development Discipline','06 / Portfolio Context','SPECIALIST OPERATING VENTURE','DEVELOPMENT-STAGE DIGITAL PRODUCT','ACTIVE BUILD','a specialist venture within the Unique Holding portfolio','Build and Test are the current development zone','FUTURE GATE'])if(!venturesMain.includes(token))errors.push('ventures.html: Phase 15 required status/IA token missing -> '+token);
  if((venturesMain.match(/data-venture-state="operating"/g)||[]).length!==1||(venturesMain.match(/data-venture-state="development"/g)||[]).length!==1)errors.push('ventures.html: Phase 15 must expose exactly one operating and one development portfolio state');
  if((venturesMain.match(/data-stage-state="current"/g)||[]).length!==2||(venturesMain.match(/data-stage-state="future"/g)||[]).length!==2)errors.push('ventures.html: YEKI HAST current/future path semantics regression');
  const yeki=venturesMain.match(/<section class="subsite-section alt ventures-section" id="yeki-hast-status">[\s\S]*?<\/section>/i)?.[0]||'';
  for(const token of ['DEVELOPMENT-STAGE DIGITAL PRODUCT','ACTIVE BUILD','structured human-listening service workflows','current product development and testing work','Technology Context','Corporate Contact'])if(!yeki.includes(token))errors.push('ventures.html: YEKI HAST public boundary missing -> '+token);
  for(const rx of [/\blaunched\b/i,/\blive now\b/i,/\bavailable now\b/i,/\bproduction[- ]ready\b/i,/\bmarket[- ]ready\b/i,/\bactive users\b/i,/\bcommunity\b/i,/\bcustomers\b/i,/\bdownloads\b/i,/\brevenue\b/i,/\btransactions\b/i,/\btraction\b/i,/\bvalidated\b/i,/\bsuccessful launch\b/i,/\bgrowing\b/i,/\bscaled\b/i,/platform uptime/i,/global service/i,/\bmarketplace\b/i,/\bleading\b/i,/award-winning/i,/\bfunded\b/i,/\binvestment\b/i,/portfolio company/i])if(rx.test(venturesMain))errors.push('ventures.html: unsupported Phase 15 launch/traction/fund wording -> '+rx);
  for(const rx of [/\bVercel\b/i,/\bNeon\b/i,/\bSMTP\b/i,/\bOTP\b/i,/\bdatabase\b/i,/\bmigration\b/i,/\brollback\b/i,/\bGitHub\b/i,/project id/i,/\bcredential/i,/\bDB region\b/i,/\bRunner\b/i,/\btest count\b/i,/payment provider/i])if(rx.test(venturesMain))errors.push('ventures.html: private YEKI HAST engineering detail leaked -> '+rx);
  for(const rx of [/Open YEKI/i,/Try YEKI/i,/Join YEKI/i,/Download App/i,/Get Started/i,/Waitlist/i])if(rx.test(venturesMain))errors.push('ventures.html: fake YEKI HAST public CTA returned -> '+rx);
  if(/feature-image\s+tech/i.test(venturesMain)||/<img\b/i.test(venturesMain))errors.push('ventures.html: unverified/fake venture visual must not be rendered in Phase 15');
  if(!venturesCss.includes('.ventures-body .subsite-hero.tech:before{background-image:none!important'))errors.push('Phase 15: inherited remote ventures hero must be explicitly disabled');
  if(/https?:\/\/|url\s*\(/i.test(venturesCss))errors.push('assets/ventures.css: remote/image dependency forbidden');
  for(const href of ['evidence-axis.html','technology.html','corporate.html','contact.html'])if(!venturesMain.includes('href="'+href+'"'))errors.push('ventures.html: required Phase 15 route missing -> '+href);
  if(/https?:\/\//i.test(venturesMain))errors.push('ventures.html: no external or fake YEKI HAST public route is approved in Phase 15');
  if(/YEKI HAST[\s\S]{0,900}\b(?:company|subsidiary|division|department|business unit|registered venture)\b/i.test(venturesMain))errors.push('ventures.html: invented YEKI HAST legal relationship wording');
  if(!ventures.includes('<title>Venture Development | Unique Holding</title>'))errors.push('ventures.html: Phase 15 title regression');
  if(!ventures.includes('rel="canonical" href="https://www.uniqueholding.com.tr/ventures.html"'))errors.push('ventures.html: Phase 15 canonical regression');
  if(!ventures.includes('name="description" content="Venture development at Unique Holding: Evidence Axis as a specialist operating venture and YEKI HAST as a development-stage digital product under active build."'))errors.push('ventures.html: Phase 15 meta description regression');
  const auditPath='docs/qa/phase15-yeki-hast-public-evidence.md';if(!files.includes(auditPath))errors.push('Phase 15 YEKI HAST evidence audit missing');else{const audit=read(auditPath);for(const token of ['Supported internally?','Appropriate to publish?','KEEP','REMOVE','UNSAFE','SAFE','code existence does not establish public availability'])if(!audit.includes(token))errors.push('Phase 15 YEKI HAST evidence audit incomplete -> '+token)}
  const axis=read('evidence-axis.html');const axisCss=read('assets/evidence-axis-bridge.css');for(const token of ['01 / Evidence Axis','SPECIALIST VENTURE','a specialist venture within the Unique Holding portfolio','View Public Sample','Open EvidenceAxis.com'])if(!axis.includes(token))errors.push('Phase 15: Phase 14 Evidence Axis lock regression -> '+token);if(!axisCss.includes('.evidence-axis-bridge-body .subsite-hero.tech:before{background-image:none!important'))errors.push('Phase 15: Phase 14 Evidence Axis visual lock regression');
  const technology=read('technology.html');const technologyVisualCss=read('assets/technology-visuals.css');for(const token of ['technology-hero-system.svg','evidence-axis-system.svg','digital-product-development.svg','venture-systems.svg','content-distribution-system.svg'])if(!technology.includes(token))errors.push('Phase 15: Phase 13 Technology visual lock missing -> '+token);if(/images\.unsplash\.com/i.test(technologyVisualCss))errors.push('Phase 15: Phase 13 Technology remote visual regression');
  const sales=read('sales.html');for(const token of ['01 / Product','02 / Grade / Specification','03 / Quantity','04 / Destination','05 / Timing','06 / Specification / Technical Requirement','07 / Contact','sales@uniqueholding.com.tr'])if(!sales.includes(token))errors.push('Phase 15: Phase 11 RFQ lock regression -> '+token);
  const p09=read('products.html');const refs=(p09.match(/data-destination="reference-detail"/g)||[]).length;const inquiries=(p09.match(/data-destination="inquiry-detail"/g)||[]).length;if(refs!==3||inquiries!==62)errors.push('Phase 15: Phase 09 3/62 classification regression');
...
// Phase 16 — Contact routing / verified external identity guards.
{
  ...
  for(const token of ['29 Ekim Cad. Yenibosna Merkez Mah.','İstanbul Vizyon Park Plazaları A1 Blok','Kat: 9, Daire: 98','Bahçelievler / İstanbul, Türkiye','+90 212 727 22 22','sales@uniqueholding.com.tr'])if(!main.includes(token))errors.push('contact.html: approved public contact data missing -> '+token);
  const uniqueEmails=[...main.matchAll(/(?:mailto:)?([A-Z0-9._%+-]+@uniqueholding\.com\.tr)/gi)].map(m=>m[1].toLowerCase());for(const email of new Set(uniqueEmails))if(!['sales@uniqueholding.com.tr'].includes(email))errors.push('contact.html: invented/unapproved Unique Holding email published -> '+email);
  ...
  if(!main.includes('tel:+902127272222'))errors.push('contact.html: approved office tel route missing');
  for(const token of ['subject=Industrial%20Sales%20Inquiry%20%E2%80%94%20Unique%20Holding'])if(!main.includes(token))errors.push('contact.html: useful encoded mailto subject missing -> '+token);
...
// Phase 17 — privacy / legal factual-alignment guards.
{
  const privacy=read('privacy.html');const legalNotice=read('legal.html');const privacyMain=privacy.match(/<main\b[\s\S]*?<\/main>/i)?.[0]||'';const legalMain=legalNotice.match(/<main\b[\s\S]*?<\/main>/i)?.[0]||'';const legalCss=read('assets/legal.css');
  if(!privacy.includes('assets/legal.css')||!legalNotice.includes('assets/legal.css')||!legalCss.startsWith('@layer page{'))errors.push('Phase 17: legal/privacy stylesheet architecture missing');
  for(const page of [['privacy.html',privacyMain],['legal.html',legalMain]])if((page[1].match(/<h1\b/gi)||[]).length!==1)errors.push(page[0]+': Phase 17 requires exactly one H1');
  for(const token of ['01 / Privacy &amp; Cookies','02 / Current Website Data Flows','03 / Direct Business Correspondence','04 / Cookies / Browser Storage','05 / Technical Delivery / External Requests','06 / External Destinations','07 / Privacy Contact','08 / Legal Review Boundaries / Last Updated','1 September 2026'])if(!privacyMain.includes(token))errors.push('privacy.html: Phase 17 IA token missing -> '+token);
  for(const token of ['01 / Legal Notice','02 / Corporate Identity','03 / Website / Commercial Information','04 / Product / Offered-Lot Boundary','05 / Visual Ownership Boundary','06 / Portfolio Identity Boundaries','07 / External Destinations / Website Content','08 / Contact / Last Updated','1 September 2026'])if(!legalMain.includes(token))errors.push('legal.html: Phase 17 IA token missing -> '+token);
  const company='UNIQE OTOMOTİV KİMYA SANAYİ LİMİTED ŞİRKETİ';const address=['29 Ekim Cad. Yenibosna Merkez Mah.','İstanbul Vizyon Park Plazaları A1 Blok','Kat: 9, Daire: 98','Bahçelievler / İstanbul, Türkiye'];for(const block of [['privacy/legal shared identity',legalMain]]){if(!block[1].includes(company))errors.push('legal.html: approved operating-company spelling missing');for(const t of address)if(!block[1].includes(t))errors.push('legal.html: approved address token missing -> '+t)}
  for(const t of ['+90 212 727 22 22','sales@uniqueholding.com.tr'])if(!legalMain.includes(t))errors.push('legal.html: approved contact missing -> '+t);
  if(!privacyMain.includes('+90 212 727 22 22'))errors.push('privacy.html: current public office contact point missing');
  const publicSweep=htmlFiles.map(f=>[f,read(f)]);const unsupported=[/KVKK compliant/i,/GDPR compliant/i,/fully compliant/i,/privacy compliant/i,/secure transmission/i,/secure website/i,/encrypted inquiry/i,/we never share/i,/we never transfer/i,/we never retain/i,/data is never stored/i,/all data deleted/i,/\banonymized\b/i,/no personal data processed/i,/no cookies ever/i,/no tracking ever/i];for(const [f,h] of publicSweep)for(const rx of unsupported)if(rx.test(h))errors.push(f+': unsupported Phase 17 compliance/privacy absolute -> '+rx);
  const boiler=[/use at your own risk/i,/no warranties whatsoever/i,/all liability excluded/i,/exclusive jurisdiction/i,/binding arbitration/i,/indemnification/i,/force majeure/i,/terms acceptance by browsing/i];for(const [f,h] of publicSweep)for(const rx of boiler)if(rx.test(h))errors.push(f+': unauthorized generic legal boilerplate -> '+rx);
  for(const rx of [/\bMERS[Iİ]S\b/i,/trade registry number/i,/tax number/i,/vergi numara/i,/\bVKN\b/i,/registered capital/i,/\bDPO\b/i,/privacy officer/i,/statutory representative/i])if(rx.test(privacyMain)||rx.test(legalMain))errors.push('Phase 17: invented/unapproved legal identity detail -> '+rx);
  for(const rx of [/\bis (?:the )?data controller\b/i,/\bacts as (?:the )?data controller\b/i,/\bveri sorumlusudur\b/i])if(rx.test(privacyMain))errors.push('privacy.html: unsupported settled data-controller statement -> '+rx);
  for(const rx of [/legitimate interest/i,/contract necessity/i,/legal obligation as the legal basis/i,/explicit consent as the legal basis/i,/standard contractual clauses/i,/adequacy decision/i,/binding corporate rules/i])if(rx.test(privacyMain))errors.push('privacy.html: unreviewed legal basis/transfer mechanism -> '+rx);
  if(/\b\d+\s+(?:day|days|month|months|year|years)\b/i.test(privacyMain))errors.push('privacy.html: invented fixed retention period');
  if(/Some pages also load externally hosted photography/i.test(privacyMain))errors.push('privacy.html: stale generic external-photography wording returned');
  for(const token of ['No website-set cookie or browser-storage item was observed','images.unsplash.com','Home, Energy &amp; Trade, Products and Industrial Sales','No third-party script, remote font, third-party embed, external media stream, analytics request, advertising request, XHR, fetch or beacon was observed'])if(!privacyMain.includes(token))errors.push('privacy.html: Phase 17 current technical disclosure missing -> '+token);
  if(!privacyMain.includes('not presented as a formal KVKK Article 10 data-controller notice')||!privacyMain.includes('Legal review required'))errors.push('privacy.html: formal-notice limitation must remain explicit');
  const sales=read('sales.html');for(const token of ['This page does not submit or store an RFQ on a server','The builder keeps the information in your browser and prepares an email addressed to sales@uniqueholding.com.tr','Email handoff — not server submission','mailto:sales@uniqueholding.com.tr'])if(!sales.includes(token))errors.push('Phase 17: Phase 11 RFQ privacy handoff lock regression -> '+token);if(/XMLHttpRequest|navigator\.sendBeacon|\bfetch\s*\(/.test(sales))errors.push('sales.html: RFQ gained an unapproved network-submission mechanism');
  const relationship='Evidence Axis is a specialist venture within the Unique Holding portfolio, focused on competitive intelligence, B2B SaaS competitive research and decision-oriented research.';if(!legalMain.includes(relationship))errors.push('legal.html: Evidence Axis exact relationship wording regression');
  if(!legalMain.includes('DEVELOPMENT-STAGE DIGITAL PRODUCT'))errors.push('legal.html: YEKI HAST maturity lock missing');
  for(const rx of [/Evidence Axis[^.]{0,120}\b(?:subsidiary|division|department|business unit|wholly owned company)\b/i,/YEKI HAST[^.]{0,120}\b(?:company|subsidiary|division|department|business unit|launched service)\b/i])if(rx.test(legalMain))errors.push('legal.html: unsupported venture legal/status wording -> '+rx);
  if(!legalMain.includes('not a binding commercial offer')||!legalMain.includes('No standing inventory statement')||!legalMain.includes('Visual presentation is not evidence of asset ownership'))errors.push('legal.html: commercial/visual boundary regression');
  for(const asset of ['docs/qa/phase17-privacy-legal-audit.md','docs/qa/phase17-site-data-flow-inventory.md'])if(!files.includes(asset))errors.push('Phase 17 internal audit missing -> '+asset);
  if(files.includes('docs/qa/phase17-privacy-legal-audit.md')){const a=read('docs/qa/phase17-privacy-legal-audit.md');for(const t of ['LEGAL-PRIVACY-001','LEGAL-PRIVACY-002','LEGAL REVIEW REQUIRED','Official KVKK references consulted','2022/1358','2024/1361','2020/71'])if(!a.includes(t))errors.push('Phase 17 legal audit incomplete -> '+t)}
  if(files.includes('docs/qa/phase17-site-data-flow-inventory.md')){const inv=read('docs/qa/phase17-site-data-flow-inventory.md');for(const t of ['32 CLEAN-CONTEXT CASES','COOKIES OBSERVED: 0','LOCALSTORAGE KEYS: 0','SESSIONSTORAGE KEYS: 0','INDEXEDDB DATABASES: 0','SERVICE WORKERS: 0','ANALYTICS / ADVERTISING REQUESTS: 0','XHR / FETCH / BEACON: 0','images.unsplash.com','sales.html','RFQ browser / mailto handoff'])if(!inv.includes(t))errors.push('Phase 17 data-flow inventory incomplete -> '+t)}
  for(const href of ['legal.html','contact.html'])if(!privacyMain.includes('href="'+href+'"'))errors.push('privacy.html: required Phase 17 link missing -> '+href);for(const href of ['privacy.html','sales.html','contact.html'])if(!legalMain.includes('href="'+href+'"'))errors.push('legal.html: required Phase 17 link missing -> '+href);
  const contact=read('contact.html');for(const token of ['INDUSTRIAL SALES','CORPORATE COMMUNICATION','EVIDENCE AXIS','TECHNOLOGY / VENTURE','06 / Istanbul Office'])if(!contact.includes(token))errors.push('Phase 17: Phase 16 Contact lock regression -> '+token);
  const ventures=read('ventures.html');if(!ventures.includes('SPECIALIST OPERATING VENTURE')||!ventures.includes('DEVELOPMENT-STAGE DIGITAL PRODUCT'))errors.push('Phase 17: Phase 15 Ventures lock regression');
  const axis=read('evidence-axis.html');if(!axis.includes('a specialist venture within the Unique Holding portfolio'))errors.push('Phase 17: Phase 14 Evidence Axis lock regression');
  const technology=read('technology.html');for(const token of ['technology-hero-system.svg','evidence-axis-system.svg','digital-product-development.svg','venture-systems.svg','content-distribution-system.svg'])if(!technology.includes(token))errors.push('Phase 17: Phase 13 Technology visual lock missing -> '+token);
  const p09=read('products.html');const refs=(p09.match(/data-destination="reference-detail"/g)||[]).length;const inquiries=(p09.match(/data-destination="inquiry-detail"/g)||[]).length;if(refs!==3||inquiries!==62)errors.push('Phase 17: Phase 09 3/62 classification regression');
  const releaseBlocker={id:'RELEASE-BLOCKER-HTTPS-001',status:'OPEN'};if(releaseBlocker.status!=='OPEN')errors.push('Phase 17: RELEASE-BLOCKER-HTTPS-001 must remain OPEN');
}

if(errors.length){console.error('\nTECHNICAL QA FAILED');for(const e of errors)console.error(' - '+e);process.exit(1)}
console.log('TECHNICAL QA PASS — '+htmlFiles.length+' HTML, '+jsFiles.length+' JS, '+cssFiles.length+' CSS files checked; '+urls.length+' sitemap URLs verified; Phase 09/10/11/12/13/14/15/16/17 guards passed; protected film hashes verified.');
