...
if '## 67. PERF-QA-MEASUREMENT-003 — PRODUCTS MOBILE' not in text:
    ...
    extra+=f"## 75. CLASSIFIER FIXTURE RESULTS\n\nFixtures: **{fixtures.get('passed',0)}/{fixtures.get('fixtureCount',0)} PASS**, failures: **{fixtures.get('failed',0)}**.\n\n"+'\n'.join(fixture_lines)+'\n\n'
    ...
    final_pass=(perf.get('unresolvedCount',0)==0 and len(acc.get('errors',[]))==0 and fixtures.get('failed',1)==0)
    extra+=f"## 80. FINAL PHASE 19 ACCEPTANCE RESULT\n\n**{'PASS — READY FOR EXTERNAL REVIEW' if final_pass else 'FAIL — NOT READY'}**. Classifier fixtures: {fixtures.get('passed',0)}/{fixtures.get('fixtureCount',0)}. Axe: `{acc.get('axe')}`. Browser regression failures: {acc.get('regression',{}).get('totalFailures')}. Performance unresolved: {perf.get('unresolvedCount',0)}. Final acceptance errors: {len(acc.get('errors',[]))}.\n"
