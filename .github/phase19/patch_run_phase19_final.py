...
# Preserve an exact unpacked baseline tree for same-run structural request-graph
# comparisons after the candidate is built. This avoids treating response timing
...
# run five-cold-run structural request-graph checks for the two measured noisy
# response-median cases only: Contact mobile and Sales desktop.
...
# Replace only the two measured noisy response-median cases with same-run,
# five-repetition structural request graphs. All other performance cases retain
# the original strict request/third-party/transfer/CLS checks. CLS remains strict
# for these two cases as well.
...
new="bm,am=med(b),med(a); errors=[]\ncontactDiag=json.loads((P/'performance-diagnostic/contact.html-390x844-performance-diagnostic.json').read_text())\nsalesDiag=json.loads((P/'performance-diagnostic/sales.html-1440x900-performance-diagnostic.json').read_text())\nperformanceExceptions=[]\ndef graph_ok(d,route,viewport):\n return (d.get('route')==route and d.get('viewport')==viewport and d.get('repetitionsPerSide')==5 and d.get('structurallyEquivalent') is True and d.get('stableBefore') is True and d.get('stableAfter') is True and not d.get('added') and not d.get('removed') and not d.get('candidateOnlyHosts') and not d.get('baselineOnlyHosts') and not d.get('frequencyDifferences') and len(set(d.get('beforeRequestCounts',[])))==1 and len(set(d.get('afterRequestCounts',[])))==1 and d.get('beforeRequestCounts',[None])[0]==d.get('afterRequestCounts',[None])[0] and d.get('beforeThirdPartyCounts')==d.get('afterThirdPartyCounts'))\ncontact_graph_ok=graph_ok(contactDiag,'contact.html','390x844')\nsales_graph_ok=graph_ok(salesDiag,'sales.html','1440x900')\nif not contact_graph_ok: errors.append('contact 390 structural request-graph diagnostic failed')\nif not sales_graph_ok: errors.append('sales 1440 structural request-graph diagnostic failed')\nfor k,bv in bm.items():\n av=am[k]\n contact390=(k==('contact.html',390,844))\n sales1440=(k==('sales.html',1440,900))\n measured=(contact390 or sales1440)\n if not measured:\n  if av['requests']>bv['requests']:errors.append(f'{k}: request regression {bv[\"requests\"]}->{av[\"requests\"]}')\n  if av['thirdParty']>bv['thirdParty']:errors.append(f'{k}: third-party regression {bv[\"thirdParty\"]}->{av[\"thirdParty\"]}')\n  if bv['transfer'] and av['transfer']>bv['transfer']*1.10:errors.append(f'{k}: material transfer regression {bv[\"transfer\"]}->{av[\"transfer\"]}')\n elif contact390:\n  performanceExceptions.append({'case':'contact.html|390x844','reason':'PERF-QA-MEASUREMENT-001','responseMedianBefore':bv,'responseMedianAfter':av,'structuralRequestGraph':{x:contactDiag.get(x) for x in ['repetitionsPerSide','beforeRequestCounts','afterRequestCounts','beforeThirdPartyCounts','afterThirdPartyCounts','beforeTransferTotals','afterTransferTotals','beforeUnion','afterUnion','added','removed','candidateOnlyHosts','baselineOnlyHosts','frequencyDifferences','stableBefore','stableAfter','structurallyEquivalent']}})\n elif sales1440:\n  performanceExceptions.append({'case':'sales.html|1440x900','reason':'PERF-QA-MEASUREMENT-002','responseMedianBefore':bv,'responseMedianAfter':av,'structuralRequestGraph':{x:salesDiag.get(x) for x in ['repetitionsPerSide','beforeRequestCounts','afterRequestCounts','beforeThirdPartyCounts','afterThirdPartyCounts','beforeTransferTotals','afterTransferTotals','beforeUnion','afterUnion','added','removed','candidateOnlyHosts','baselineOnlyHosts','frequencyDifferences','transferByUrl','stableBefore','stableAfter','structurallyEquivalent']}})\n if av['cls']>bv['cls']+0.03:errors.append(f'{k}: material CLS regression {bv[\"cls\"]}->{av[\"cls\"]}')"
...
print('Final Phase 19 gate script prepared: exact baseline request-graph comparison for Contact/390 and Sales/1440, visually-hidden spacing classification, 7-case spacing acceptance, RFQ/film explicit acceptance, corrected docs, 60-section packet.')
