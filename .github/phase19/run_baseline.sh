...
# Compact review-friendly summaries.
perf={}
for x in r['performance']:
 k=f"{x['route']}|{x['w']}x{x['h']}"
 perf.setdefault(k,[]).append(x)
rows=[]
...
# Axe rollup and exact rule occurrences.
roll={}
for a in r['accessibility']:
 for v in a['violations']:
  key=(v['impact'] or 'unknown')+'|'+v['id']
  roll.setdefault(key,{'impact':v['impact'],'id':v['id'],'contexts':0,'nodes':0,'routes':set()})
  z=roll[key];z['contexts']+=1;z['nodes']+=len(v['nodes']);z['routes'].add(a['route'])
out=[]
...
for c in r['media']:
 for x in c['items']:
  u=x['url']; q=m.setdefault(u,{'url':u,'kind':x['kind'],'routes':set(),'contexts':0,'aboveFold':False,'natural':x['natural'],'rendered':[]})
  q['routes'].add(c['route']);q['contexts']+=1;q['aboveFold']=q['aboveFold'] or x['aboveFold'];q['rendered'].append(x['rendered'])
mo=[]
